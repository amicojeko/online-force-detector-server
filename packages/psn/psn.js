...
Psn = function(){
	var options = { // Default configuration
				debug      : true,
				email      : '',
				password   : '',
				region     : 'us',
	...
	var regions		= ["us","ca","mx","cl","pe","ar","co","br","gb","ie","be","lu","nl","fr","de","at","ch","it","pt","dk","fi","no","se","au","nz","es","ru","ae","za","pl","gr","sa","cz","bg","hr","ro","si","hu","sk","tr","bh","kw","lb","om","qa","il","mt","is","cy","in","ua","hk","tw","sg","my","id","th","jp","kr"]; // Know SONY's servers
	var languages	= ["ja","en","en-GB","fr","es","es-MX","de","it","nl","pt","pt-BR","ru","pl","fi","da","no","sv","tr","ko","zh-CN","zh-TW"]; // All languages SONY accepts as parameter
	var request 	= Npm.require('request').defaults({jar: true}); // We set jar to true to enable cookie saving (Only used for the login process)
	var cheerio		= Npm.require('cheerio'); // Now required to extract the value of the input "brandingParams"
	...
	var userAgent     = 'Mozilla/5.0 (Linux; U; Android 4.3; ' + options.npLanguage + '; C6502 Build/10.4.1.B.0.101) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30 PlayStation App/2.55.8/' + options.npLanguage + '/' + options.npLanguage;
	var requestedWith = 'com.scee.psxandroid';
	var accessToken   = '';
	var refreshToken  = '';
	var codeRegex     = /redirect\?code=(.{6})/;
  var refreshInterval;
	...
	* @param 	Function 	callback 	- Calls this function once the request is complete
	*/
	this.getProfile = function (psnid, callback) {
	...
	* @param 	Function 	callback 	- Calls this function once the request is complete
	*/
	this.getFriends = function (psnid, callback) {
		if (accessToken.length > 1) {
	...
	* @param 	Function 	callback 	- Calls this function once the request is complete
	*/
	this.getTrophies = function (psnid, iconsize, offset, limit, callback) {
		if (accessToken.length > 1) {
	...
	* @param 	Function 	callback 	- Calls this function once the request is complete
	*/
	this.getGameTrophyGroups = function (psnid, npCommID, callback) {
		if (accessToken.length > 1) {
	...
	* @param 	Function 	callback 	- Calls this function once the request is complete
	*/
	this.getGameTrophies = function (psnid, npCommID, groupId, callback) {
		if (accessToken.length > 1) {
	...
	* @param 	Function 	callback 	- Calls this function once the request is complete
	*/
	this.getTrophy = function (psnid, npCommID, groupId, trophyID, callback) {
		if (accessToken.length > 1) {
